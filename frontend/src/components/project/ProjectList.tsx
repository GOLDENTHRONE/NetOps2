...
// becomes ONE application instance (except system / infrastructure namespaces,
// see isSystemNamespace). Each row is scoped to exactly one cluster and one
// namespace, so a namespace that exists in several clusters produces a separate
// row per cluster, each with its own independently computed health.
//
...
/**
 * Route params for the application details page.
 *
 * The details route is two explicit segments (/project/:cluster/:name) so a
 * cluster-scoped id never has to be squeezed through a single URL parameter.
 *
 * @param project - The application instance (exactly one cluster + namespace).
 * @returns The cluster and namespace route parameters.
 */
export function projectDetailsParams(project: ProjectDefinition) {
  return { cluster: project.clusters[0] ?? '', name: project.namespaces[0] ?? '' };
}
        ...
        Cell: ({ row: { original } }) => (
          <>
            <Link routeName="projectDetails" params={projectDetailsParams(original)}>
              {original.namespaces[0] ?? original.id}
            </Link>
          </>
        ),
      },
