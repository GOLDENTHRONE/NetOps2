...
function AuthRoute(props: AuthRouteProps) {
  ...
  function getRenderer({ location }: RouteProps) {
    ...
    if (query.isError) {
      // Redirect to /login so AuthChooser can show the "Failed to connect / Try
      // Again" dialog for transient errors (5xx/timeout) and the token/OIDC
      // flow for explicit auth failures. AuthChooser handles transient errors
      // without looping back here.
      return (
        <Redirect
          to={{
